package com.xingtao.xingtaomall.seckill.service.impl;
...
import com.xingtao.xingtaomall.seckill.feign.CouponFeignService;
import com.xingtao.xingtaomall.seckill.feign.ProductFeignService;
import com.xingtao.xingtaomall.seckill.interceptor.LoginUserInterceptor;
import com.xingtao.xingtaomall.seckill.service.SeckillService;
import com.xingtao.xingtaomall.seckill.to.SeckillSkuRedisTo;
import com.xingtao.xingtaomall.seckill.vo.SeckillSessionWithSkusVo;
import com.xingtao.xingtaomall.seckill.vo.SkuInfoVo;
 ...
 * @Description
 * @Version
 * @BelongsPackage com.xingtao.xingtaomall.seckill.service.impl
...
public class SeckillServiceImpl implements SeckillService {
    ...
    public List<SeckillSkuRedisTo> getCurrentSeckillSkus() {
        try (Entry entry = SphU.entry("seckillSkus")) {
            ...
            //从Redis中查询到所有key（以seckill:sessions开头）的所有数据
            Set<String> keys = redisTemplate.keys(SESSION__CACHE_PREFIX + "*");
            for (String key : keys) {
                //seckill:sessions:1594396764000_1594453242000
                String replace = key.replace(SESSION__CACHE_PREFIX, ""); //将seckill:sessions:替换为""
                ...
                if (currentTime >= startTime && currentTime <= endTime) {
                    ...
                    if (listValue != null && listValue.size() >= 0) {
                        ...
                        List<SeckillSkuRedisTo> collect = listValue.stream().map(item -> {
                            ...
                            // redisTo.setRandomCode(null);当前秒杀开始需要随机码，不开始就不需要
                            return redisTo;
        ...
        if (keys != null && keys.size() > 0) {
            ...
            for (String key : keys) {
                ...
                if (Pattern.matches(reg,key)) {
                    ...
                    redisTo.setRandomCode(null);  //没有在秒杀时间内，随机码置为空  p319
                    return redisTo;
                }
            }
        }
     ...
     * @return
     */
    //TODO 视频里没有：上架秒杀商品的时候，每一个数据都有应该有时间
    //TODO 视频里没有：简化了收获信息的内容
    @Override
            ...
            if (randomCode.equals(key) && killId.equals(skuId)) {
                ...
                if (count > 0 && num <= seckillLimit && count > num ) {
                    ...
                    if (aBoolean) {
                        ...
                        //TODO 秒杀成功，快速下单，发送MQ，已完成
                        boolean semaphoreCount = false;
                        try {
                            //acquire是阻塞的，tryAcquire非阻塞
                            semaphoreCount = semaphore.tryAcquire(num, 100, TimeUnit.MILLISECONDS);
    ...
    private void saveSessionSkuInfo(List<SeckillSessionWithSkusVo> sessions) {
        ...
        sessions.stream().forEach(session -> {
            ...
            session.getRelationSkus().stream().forEach(seckillSkuVo -> {
                ...
                if (!operations.hasKey(redisKey)) {   //如果redis中没有，再操作。如果有的话就不操作
